...
(define (rf-uf groups rf-pairs)
  ...
  ;; Only difference is we check if the two dst belong to the same eid-group
  ...
  (define (helper groups-h rf-pairs-h)
    (if (null? rf-pairs-h)
        ...
        (let* ((pair (car rf-pairs-h))
               ...
               (src1 (caadr pair))
               (src2 (cadadr pair))
               (c (find-group groups-h dst1))
               (d (find-group groups-h dst2))
               (flag (eq? c d))
...
(define (remove-data-dep-writes eid-partition data-dst)
  (map (lambda (group)
         (filter (lambda (el)
                   (not (member el data-dst)))
                 group))
       eid-partition))
...
(define (generate-constraints events edges is-acyclic)
  (let* ((event-names (map event->symbol events))
         ...
         (rf-dst-events (update-rf-properties events eid-partition edges))
    ...
    (apply append (list
                   ...
                   (exists-condition-constraints rf-dst-events 'ass)
                   ...
                   (map (lambda (edge)
                          (let ((name (edge-name edge))
                                ...
                                (dst (event->symbol (edge-dst edge)))
                                (type (string->symbol (edge-type edge))))
                            `(assert (= ,name (mk-edge ,src ,dst ,type)))))
                        edges)
...
(define (main args)
  ...
  (let* ((expr (cadr args))
    ...
    (let* ((expr (parse-expr expr))
           ...
           (events (sort (unique (flatten (map (lambda (edge) (list (edge-dst edge) (edge-src edge))) edges)))))
      ...
      #;(let ((parts (get-eid-partition events edges is-acyclic)))
          (define (name-of eid)
            (let ((x (map (lambda (s) (member eid s)) parts)))
              (car (map cdr (filter (lambda (p) (car p)) (map cons x (seq (length x))))))))
          (pretty-print parts)
          (for-each (lambda (ed)
                      ;source of edge
                      (pretty-print ed
